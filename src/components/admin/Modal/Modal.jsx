...
export default function Modal({active, setActive,oneCategory,updateName,setUpdateName,handleFileChange,image,updateHandler}) {
    ...
    return <div className={active? "modal active":"modal"} onClick={()=>setActive(false)}>
        <div className="modal__content flex" onClick={(e)=> e.stopPropagation()}>
            <h2>Category Name: <span><input type="text"  value={updateName} onChange={(e)=>setUpdateName(e.target.value)}/></span></h2>
            <img src={oneCategory?.photo} alt="" width="300px" height="200px"/>
            <label htmlFor="dropzone-file">File</label>
            <div class="flex items-center justify-center w-full">
                ...
                <button onClick={()=>updateHandler(oneCategory._id)}>Send</button>
        </div>
    </div>
}